...
class ControladorSucursal extends Controller
{
    ...
    public function guardar(Request $request)
    {
        try {
            ...
            //validaciones
            if ($entidad->telefono == "" || $entidad->descripcion == "" || $entidad->linkmap == "") {
    ...
    public function cargarGrilla()
    {
        ...
        for ($i = $inicio; $i < count($aSucursal) && $cont < $registros_por_pagina; $i++) {
            $row = array();
            $row[] = '<a href="/admin/sucursal/' . $aSucursal[$i]->idsucursales . '" class="btn btn-secondary"><i class="far fa-edit"></i></a>';
            $row[] = $aSucursal[$i]->telefono;
            $row[] = $aSucursal[$i]->descripcion;
