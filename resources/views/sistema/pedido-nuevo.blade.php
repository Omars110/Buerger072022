        ...
        <form id="form1" method="POST">
            <div class="row">
                ...
                <div class="form-group col-lg-6">
                    <label>Fecha:*</label>
                    <input type="date" id="txtFecha" name="txtFecha" class="form-control" value="{{ $pedido->fecha }}" style = "width:200px" required>
                </div>
                <div class="form-group col-lg-6">
                ...
                <div class="form-group col-lg-6">
                    ...
                    <select name="lstSucursal" id="lstSucursal" class="form-control" value="">
                        ...
                        @if ($sucursal->idsucursales == $pedido->fk_idsucursal)
                             <option value="{{ $sucursal->idsucursales }}" selected>{{ $sucursal->descripcion }}</option>
                             @else 
                             <option value="{{ $sucursal->idsucursales }}">{{ $sucursal->descripcion }}</option>
                        @endif
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group col-lg-6">
                    ...
                    <select name="lstEstado" id="lstEstado" class="form-control" name="" id="">
                        ...
                        @foreach($aEstados as $estado)
                            @if ($estado->idestados == $pedido->fk_estado)
                            <option value="{{ $estado->idestados }}" selected>{{ $estado->nombre }}</option>
                            @else
